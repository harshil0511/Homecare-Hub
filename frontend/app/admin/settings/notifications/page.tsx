...
import { Bell, Wrench, Calendar, Users, Mail } from "lucide-react";
...
export default function AdminNotificationsPage() {
    const [prefs, setPrefs] = useState(() => {
        try {
            const saved = localStorage.getItem("hc_admin_notification_prefs");
    ...
    const togglePref = (key: keyof typeof prefs) => {
        ...
        setPrefs(updated);
        localStorage.setItem("hc_admin_notification_prefs", JSON.stringify(updated));
    };

    const items = [
        { key: "systemAlerts" as const, icon: Bell, label: "System Alerts", desc: "Critical system events and warnings" },
        { key: "userActivity" as const, icon: Users, label: "User Activity", desc: "New registrations and role changes" },
        { key: "providerUpdates" as const, icon: Wrench, label: "Provider Updates", desc: "Provider verification requests" },
        { key: "emailNotifications" as const, icon: Mail, label: "Email Notifications", desc: "Receive admin alerts via email" },
    ];
