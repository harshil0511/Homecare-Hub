...
export default function NotificationsPage() {
    const [prefs, setPrefs] = useState(() => {
        try {
            const saved = localStorage.getItem("hc_notification_prefs");
    ...
    const togglePref = (key: keyof typeof prefs) => {
        ...
        setPrefs(updated);
        localStorage.setItem("hc_notification_prefs", JSON.stringify(updated));
    };

    const items = [
        { key: "maintenanceAlerts" as const, icon: Wrench, label: "Maintenance Alerts", desc: "Upcoming maintenance task reminders" },
        { key: "bookingUpdates" as const, icon: Calendar, label: "Booking Updates", desc: "Service booking status changes" },
        { key: "communityNotices" as const, icon: Users, label: "Community Notices", desc: "Society announcements and updates" },
